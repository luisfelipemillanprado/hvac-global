...
export const LocationCard = ({
  ...
  return (
    <section className="shadow-panel bg-graphite grid h-full grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-4xl p-5">
      ...
      <div className="bg-ink-navy relative min-h-40 overflow-hidden rounded-3xl">
        <iframe
          title={mapTitle}
          src={mapSrc}
          loading={mapLoading}
        ...
        />
        <div className="bg-pewter/10 pointer-events-none absolute inset-0" />
      </div>
      <a
        href={mapHref}
        target={mapLinkTarget}
        rel={mapLinkRel}
        className="text-body md:text-body-md lg:text-body-lg bg-ink-navy text-pewter hover:bg-blue-slate mt-3 grid grid-flow-col items-center justify-center gap-2 rounded-2xl px-4 py-3 font-black transition"
      >
